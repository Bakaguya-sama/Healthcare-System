...
| BE-RF-002 | Sửa API build/typecheck                        | Huy   | BE-RF-001                   |     1-2d | Waiting for RF-1 entry blockers |
...
`BE-RF-002` là task kế tiếp theo dependency graph, nhưng chỉ chuyển sang `In Progress` sau khi hoàn tất hai entry blockers còn lại: local startup instructions và xác minh MongoDB replica set + Redis local/test. Không bắt đầu feature `BE-NF-*` trong RF-1; build/test safety net phải xanh trước.
