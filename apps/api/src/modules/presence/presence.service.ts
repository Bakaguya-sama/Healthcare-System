...
export class PresenceService {
  ...
  async removeActiveUser(userId: string, socketId: string) {
    ...
    if (!userSockets) {
      await this.redis.run(this.redis.client.sRem(this.userKey(userId), socketId)).catch(() => undefined);
      return false;
    }
  ...
  async getActiveUsers(userIds: string[]) {
    const checks = await Promise.all(userIds.map(async (id) => {
      ...
      const remote = await this.redis.run(this.redis.client.exists(this.userKey(id))).catch(() => 0);
  ...
  private async persist(userId: string, socketId: string) {
    await this.redis.run(this.redis.client.sAdd(this.userKey(userId), socketId)).catch(() => undefined);
    await this.redis.run(this.redis.client.expire(this.userKey(userId), this.ttlSeconds)).catch(() => undefined);
  }
}
