...
describe('ConsultationsService state machine', () => {
  function createService() {
    ...
    const users = { findById: jest.fn().mockResolvedValue({ fullName: 'Patient' }) };
    return {
      service: new ConsultationsService(model as never, notifications as never, users as never),
      consultation,
      patientId,
      doctorId,
    };
  ...
  it('moves pending request through accept, start and complete', async () => {
    ...
    expect(consultation.sessionStatus).toBe(ConsultationSessionStatus.CONFIRMED);

    await service.start(doctorId.toString(), consultation._id.toString());
    expect(consultation.sessionStatus).toBe(ConsultationSessionStatus.IN_PROGRESS);

    await service.complete(doctorId.toString(), consultation._id.toString(), {} as never);
    expect(consultation.sessionStatus).toBe(ConsultationSessionStatus.COMPLETED);
  });

  it('does not allow start before doctor accepts the request', async () => {
    const { service, consultation, doctorId } = createService();
    await expect(service.start(doctorId.toString(), consultation._id.toString())).rejects.toThrow(
