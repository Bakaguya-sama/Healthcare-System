...
import {
  CreateAiDocumentChunkDto,
  UpdateAiDocumentChunkDto,
  QueryAiDocumentChunkDto,
} from './dto/create-ai-document-chunk.dto';
...
export class AiDocumentChunksService {
  constructor(
    ...
    private chunkModel: Model<AiDocumentChunkDocument>,
  ) {}
  ...
  async create(createDto: CreateAiDocumentChunkDto): Promise<AiDocumentChunk> {
    try {
      const chunk = new this.chunkModel({
        ...
        relatedChunks:
          createDto.relatedChunks?.map((id) => new Types.ObjectId(id)) || [],
  ...
  async createMany(
    chunks: CreateAiDocumentChunkDto[],
    ...
    try {
      const chunksWithData = chunks.map((chunk) => ({
        ...
        relatedChunks:
          chunk.relatedChunks?.map((id) => new Types.ObjectId(id)) || [],
        charCount: chunk.content.length,
        tokenCount: chunk.tokenCount || 0,
        usageCount: 0,
      }));
      return (await this.chunkModel.insertMany(
        chunksWithData,
      )) as AiDocumentChunk[];
    } catch (error) {
      throw new BadRequestException(
        `Failed to create chunks: ${error.message}`,
      );
    }
  }
  ...
  async findByDocumentId(
    documentId: string,
    query: QueryAiDocumentChunkDto,
  ): Promise<{ data: AiDocumentChunk[]; total: number }> {
    const {
      page = 1,
      limit = 10,
      search,
  ...
  async findById(chunkId: string): Promise<AiDocumentChunk> {
    const chunk = await this.chunkModel
      .findById(new Types.ObjectId(chunkId))
      .exec();
    if (!chunk) {
  ...
  async update(
    chunkId: string,
    updateDto: UpdateAiDocumentChunkDto,
  ): Promise<AiDocumentChunk> {
    const chunk = await this.findById(chunkId);

    if (updateDto.relatedChunks) {
      updateDto.relatedChunks = updateDto.relatedChunks.map(
  ...
  async delete(chunkId: string): Promise<AiDocumentChunk> {
    const chunk = await this.chunkModel.findByIdAndDelete(
      new Types.ObjectId(chunkId),
    );
